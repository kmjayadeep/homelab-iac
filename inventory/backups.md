...
| --- | --- | --- | --- | --- | --- |
| `helios` Postgres databases | `pg_dump` to temp files, then Restic via hourly systemd timer | Cloudflare R2 bucket `postgres-cosmos-backup` | Yes | Medium | Covers `totp`, `planka`, `immich`, `coder`, and `taskplanner` per Nix config. Needs restore test and RPO/RTO note. |
